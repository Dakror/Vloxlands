...
for(y = -smpl; y <= smpl; y++)
	for(x = -smpl; x <= smpl; x++)
		if(x != 0.0 && y != 0.0)sum += getShadowness(vec2(x * v, y * v));
...
void main() {
	...
	vec2 texUV = vec2(x + texStart.x, y + texStart.y);
		...
		vec4 diffuse = texture2D(u_diffuseTexture, texUV) * u_diffuseColor;
	#elif defined(diffuseTextureFlag) && defined(colorFlag)
		vec4 diffuse = texture2D(u_diffuseTexture, texUV) * v_color;
	#elif defined(diffuseTextureFlag)
		vec4 diffuse = texture2D(u_diffuseTexture, texUV);
